...
parser = argparse.ArgumentParser(description='Pose Regressor Training')
parser.add_argument(
    '-j',
    '--workers',
    default=4,
...
parser.add_argument(
    '-sd',
    '--save_checkpt_filename',
    default = 'checkpoint.pt',
    type=str,
    metavar='N',
    help='saves checkpoint')
parser.add_argument(
    '-thresh',
    '--threshold',
    default=2,
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        scheduler.step()
        # train for one epoch
        train(train_loader, model, criterion_recon, criterion_MAE, optimizer, epoch, 
                scheduler, args.batch_size, args.threshold, args.wandb)

    # Save checkpoint of the trained model
    save_checkpoint(model, args.save_checkpt_filename)
                

def train(train_loader, model, criterion_recon, criterion_MAE, optimizer, epoch, 
            scheduler, batch_size, thresh, USE_WANDB=False):
        ...
        if USE_WANDB:
            if i % 20 == 0 and (epoch + 1) % 5 == 0:
                ...
                for j in range(len(og_inputs)):
                    if abs(target_phi[j] - pred_phi[j]) > 90 or abs(target_theta[j] - pred_theta[j]) > 90:
                        img1 = wandb.Image(rendered_img[j], caption="Rendered Image Azim: {} Elev: {}".format(pred_phi[j].item(), pred_theta[j].item()))
                        img2 = wandb.Image(og_inputs[j], caption="Ground Truth Image Azim:{} Elev: {}".format(target_phi[j].item(), target_theta[j].item()))
                        wandb.log({'Bad Predictions GT/pred images:' : [img2, img1]})

    
    accuracy_phi_2 = accuracy_phi_2/(len(train_loader) * batch_size)
    accuracy_phi_5 = accuracy_phi_5/(len(train_loader) * batch_size)
    accuracy_phi_10 = accuracy_phi_10/(len(train_loader) * batch_size)
    accuracy_theta_2 = accuracy_theta_2/(len(train_loader) * batch_size)
    accuracy_theta_5 = accuracy_theta_5/(len(train_loader) * batch_size)
    accuracy_theta_10 = accuracy_theta_10/(len(train_loader) * batch_size)
    
    print("Epoch: {} Threshold 2 degrees Theta Accuracy: {}  Phi Accuracy: {}".format(epoch,accuracy_theta_2, accuracy_phi_2))
    print("Epoch: {} Threshold 5 degrees Theta Accuracy: {}  Phi Accuracy: {}".format(epoch, accuracy_theta_5, accuracy_phi_5))
    print("Epoch: {} Threshold 10 degrees Theta Accuracy: {}  Phi Accuracy: {}".format(epoch, accuracy_theta_10, accuracy_phi_10))
